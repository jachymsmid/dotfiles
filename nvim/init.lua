...
local function setup_cpp_lsp()
    ...
    root_dir = find_root({ 'compile_commands.json', '.git', 'CMakeLists.txt' }),
    on_attach = on_attach,
    settings = {
      clangd = {
        -- Example: tweak some useful flags
        arguments = {
          "--background-index",
          "--clang-tidy",
...
-- This runs whenever an LSP client attaches
local on_attach = function(client, bufnr)
  ...
  vim.keymap.set("i", "<C-Space>", function()
    vim.fn.complete(vim.fn.col("."), vim.lsp.omnifunc())
  end, opts)
end
